...
\begin{itemize}
    \item \textit{"When registering, users should be logged in automatically; rather than putting in their details again"} - This could be achieved by storing the users login details after registration, then auto-filling the log in form with this data.
    ...
    \item \textit{"As an alternative to the clock, clicking show members could show a list of statuses. This would be useful in case of a large number in a group."} - When the group members are shown after pressing show members, the users status, as well as hand colour could be displayed.
...
The completion of the non-functional requirements listed in \ref{non-functional} can be assessed from these evaluation results. The applications navigation system was praised through user feedback, and received positive responses to the navigation related questions in both the in person, and online experiments. Another non-functional requirement listed was that the application should have a visually pleasing user interface throughout. The consistent, and aesthetically pleasing user interface was praised throughout with most participants responding positively to design related questions. No accessibility issues were noted, with considerations for accessibility, and usability made throughout the design, and implementation phases of the project. The application loading times cannot be statistically measured due to loading times dependent on multiple variables such as, differing devices, operating systems, and internet connectivity. However, there were no issues observed during the evaluation, or development concerning the application loading times. A wont have requirement listed was the integration of dark mode, this was not completed during this release but could be in the future with the implementation of a dark mode setting. Overall, the prioritised non-functional requirements were met, and users reacted positively to the application, therefore the product can be accepted. 
